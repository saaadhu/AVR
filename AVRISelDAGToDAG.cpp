...
/// AVRDAGToDAGISel - AVR specific code to select AVR machine
/// instructions for SelectionDAG operations.
///
namespace {
  class AVRDAGToDAGISel : public SelectionDAGISel {
    const AVRTargetLowering &Lowering;
  ...
  public:
    ...
    bool MatchAddress(SDValue N, AVRISelAddressMode &AM);
    bool MatchWrapper(SDValue N, AVRISelAddressMode &AM);
    bool MatchAddressBase(SDValue N, AVRISelAddressMode &AM);
...
/// MatchWrapper - Try to match AVRISD::Wrapper node into an addressing mode.
/// These wrap things that will resolve down into a symbol reference.  If no
/// match is possible, this returns true, otherwise it returns false.
bool AVRDAGToDAGISel::MatchWrapper(SDValue N, AVRISelAddressMode &AM) {
  return true;
  /*
...
bool AVRDAGToDAGISel::MatchAddressBase(SDValue N, AVRISelAddressMode &AM) {
  return true;
  /*
  // Is the base register already occupied?
  if (AM.BaseType != AVRISelAddressMode::RegBase || AM.Base.Reg.getNode()) {
    // If so, we cannot select it.
    return true;
  }

  // Default, generate it as a register.
  AM.BaseType = AVRISelAddressMode::RegBase;
  AM.Base.Reg = N;
  return false;
  */
}
...
bool AVRDAGToDAGISel::MatchAddress(SDValue N, AVRISelAddressMode &AM) {
  ...
  case ISD::FrameIndex:
    if (AM.BaseType == AVRISelAddressMode::RegBase
        && AM.Base.Reg.getNode() == 0) {
      AM.BaseType = AVRISelAddressMode::FrameIndexBase;
...
bool AVRDAGToDAGISel::SelectAddr(SDValue N,
  ...
  Base  = (AM.BaseType == AVRISelAddressMode::FrameIndexBase) ?
  ...
  if (AM.GV)
    ...
    Disp = CurDAG->getTargetJumpTable(AM.JT, MVT::i16, 0);
...
SDNode *AVRDAGToDAGISel::SelectIndexedLoad(SDNode *N) {
  ...
  switch (VT.SimpleTy) {
  case MVT::i8:
    Opcode = AVR::MOV8rm_POST;
    break;
  case MVT::i16:
    Opcode = AVR::MOV16rm_POST;
    break;
  default:
    return NULL;
  }
...
SDNode *AVRDAGToDAGISel::Select(SDNode *Node) {
  ...
  case ISD::FrameIndex: {
    ...
    if (Node->hasOneUse())
      return CurDAG->SelectNodeTo(Node, AVR::ADD16ri, MVT::i16,
                                  TFI, CurDAG->getTargetConstant(0, MVT::i16));
    return CurDAG->getMachineNode(AVR::ADD16ri, dl, MVT::i16,
                                  TFI, CurDAG->getTargetConstant(0, MVT::i16));
  }
  case ISD::LOAD:
  ...
  case ISD::ADD:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           ...
                           AVR::ADD8rm_POST, AVR::ADD16rm_POST))
      return ResNode;
  ...
  case ISD::SUB:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           Node->getOperand(0), Node->getOperand(1),
                           AVR::SUB8rm_POST, AVR::SUB16rm_POST))
      return ResNode;
  ...
  case ISD::AND:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           Node->getOperand(0), Node->getOperand(1),
                           AVR::AND8rm_POST, AVR::AND16rm_POST))
      return ResNode;
    else if (SDNode *ResNode =
             SelectIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                                AVR::AND8rm_POST, AVR::AND16rm_POST))
  ...
  case ISD::OR:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           Node->getOperand(0), Node->getOperand(1),
                           AVR::OR8rm_POST, AVR::OR16rm_POST))
      return ResNode;
    else if (SDNode *ResNode =
             SelectIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                                AVR::OR8rm_POST, AVR::OR16rm_POST))
  ...
  case ISD::XOR:
    if (SDNode *ResNode =
        SelectIndexedBinOp(Node,
                           Node->getOperand(0), Node->getOperand(1),
                           AVR::XOR8rm_POST, AVR::XOR16rm_POST))
      return ResNode;
    else if (SDNode *ResNode =
             SelectIndexedBinOp(Node, Node->getOperand(1), Node->getOperand(0),
                                AVR::XOR8rm_POST, AVR::XOR16rm_POST))
